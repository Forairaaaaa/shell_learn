...
SOURCES_MIRROR_DIR="sources_mirror"
...
get_country() {
    ...
    MENU_LIST=()
    for file in $(ls "${SCRIPT_DIR}/${SOURCES_MIRROR_DIR}"); do
        # echo ${file}
        MENU_LIST+=("${file}" "")
    done
    # echo ${MENU_LIST[@]}
...
get_mirror_url() {
    ...
    MENU_LIST=()
    count=0
    for url in $(grep -r ${CURRENT_DISTROS} "${SCRIPT_DIR}/${SOURCES_MIRROR_DIR}/${SELECTED_COUNTRY}"); do
        temp=$[${count}%2]
        if (( ${temp} != 0 )); then
            echo ${url}
            MENU_LIST+=("${url}" "")
