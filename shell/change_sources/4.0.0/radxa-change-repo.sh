...
cut_country_list() {
    ...
    country=$(cat "${SCRIPT_DIR}/${SOURCES_MIRROR_DIR}")
    # CURRENT_DISTROS="Debian"
    case ${CURRENT_DISTROS} in
        "Ubuntu")
            country=${country#*\[Ubuntu\]}
            country=${country%%\[Debian\]*}
            ;;
        "Debian")
            country=${country#*\[Debian\]}
            country=${country%%\[Ubuntu\]*}
...
cut_sources_list() {
    # Cut ountry info by distros
    sources=$(cat "${SCRIPT_DIR}/${SOURCES_MIRROR_DIR}")
    case ${CURRENT_DISTROS} in
        "Ubuntu")
            sources=${sources#*\[Ubuntu\]}
            sources=${sources%%\[Debian\]*}
            sources=${sources#*${SELECTED_COUNTRY}}
            sources=${sources%%@*}
            ;;
        "Debian")
            sources=${sources#*\[Debian\]}
    ...
    # Store sources mirror in menu list
    MENU_LIST=()
    count=0
